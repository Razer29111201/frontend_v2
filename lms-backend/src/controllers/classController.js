...
class ClassController {
    static async getAll(req, res) {
        try {
            ...
            const result = await Promise.all(classes.map(async (c) => {
                ...
                return {
                    id: c.id, code: c.code, name: c.name, teacher: c.teacher, teacherId: c.teacher_id,
                    cm: c.cm, cmId: c.cm_id, students: studentCount[0]?.count || 0, startDate: c.start_date,
    ...
    static async getOne(req, res) {
        try {
            ...
            res.json({
                success: true,
                data: {
                    id: cls.id, code: cls.code, name: cls.name, teacher: cls.teacher, teacherId: cls.teacher_id,
                    cm: cls.cm, cmId: cls.cm_id, students: studentCount[0]?.count || 0, startDate: cls.start_date,
    ...
    static async create(req, res) {
        try {
            const { code, name, teacher, teacherId, cm, cmId, startDate, weekDay, timeSlot, color, totalSessions } = req.body;
            ...
            const result = await transaction(async (conn) => {
                const [classResult] = await conn.query(
                    `INSERT INTO classes (code, name, teacher, teacher_id, cm, cm_id, start_date, week_day, time_slot, color, total_sessions)
                     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
                    [code, name, teacher, teacherId || null, cm, cmId || null, startDate, weekDay || 1, timeSlot, color || 'green', totalSessions || 15]
                );
            ...
            res.status(201).json({ success: true, data: { id: result.classId, code, name, teacher, teacherId, cm, cmId, startDate, weekDay, timeSlot, color, totalSessions: result.sessions.length } });
        } catch (error) {
            res.status(500).json({ success: false, error: error.message });
        }
    }
    ...
    static async update(req, res) {
        try {
            ...
            await transaction(async (conn) => {
                await conn.query(
                    `UPDATE classes SET code=?, name=?, teacher=?, teacher_id=?, cm=?, cm_id=?, start_date=?, week_day=?, time_slot=?, color=? WHERE id=?`,
                    [code, name, teacher, teacherId, cm, cmId, startDate, weekDay, timeSlot, color, id]
                );
            ...
            res.json({ success: true, data: { id: parseInt(id), code, name, teacher, teacherId, cm, cmId, startDate, weekDay, timeSlot, color } });
        } catch (error) {
            res.status(500).json({ success: false, error: error.message });
        }
    }
